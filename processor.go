...
func Process(info *types.Info, filter *PosFilter, n ast.Node, cfg *Config) (*Result, error) {
	switch x := n.(type) {
	case *ast.AssignStmt:
		return processSetterAssign(info, x)
	case *ast.IncDecStmt:
		return processSetterIncDec(info, x)
	default:
		return &Result{}, nil
	}
}

func processSetterAssign(info *types.Info, x *ast.AssignStmt) (*Result, error) {
	if x == nil {
		return &Result{}, nil
	}
	...
	op := x.Tok
	if op != token.ASSIGN && op != token.ADD_ASSIGN && op != token.SUB_ASSIGN {
		return &Result{}, nil
	}

	setterExpr, ok := buildSetterExpr(info, x.Lhs[0], true)
	if !ok {
		return &Result{}, nil
	}

	if !strings.Contains(setterExpr, ".Set") {
		return &Result{}, nil
	}

	if op == token.ASSIGN && isNil(x.Rhs[0]) {
		clearerExpr, ok := buildClearerExpr(info, x.Lhs[0])
		if ok {
			return &Result{
				From: formatNode(x.Lhs[0]),
				To:   clearerExpr + "()",
			}, nil
		}
	}

	rhs := buildSetterArgExpr(info, x.Lhs[0], x.Rhs[0])
	if op == token.ADD_ASSIGN || op == token.SUB_ASSIGN {
		currentExpr, ok := buildSetterExpr(info, x.Lhs[0], false)
		if !ok {
			return &Result{}, nil
		}
		sign := "+"
		if op == token.SUB_ASSIGN {
			sign = "-"
		}
		return &Result{
			From: formatNode(x.Lhs[0]),
			To:   fmt.Sprintf("%s(%s %s %s)", setterExpr, currentExpr, sign, rhs),
		}, nil
	}

	return &Result{
		From: formatNode(x.Lhs[0]),
		To:   fmt.Sprintf("%s(%s)", setterExpr, rhs),
	}, nil
}

func processSetterIncDec(info *types.Info, x *ast.IncDecStmt) (*Result, error) {
	if x == nil {
		return &Result{}, nil
	}

	setterExpr, ok := buildSetterExpr(info, x.X, true)
	if !ok || !strings.Contains(setterExpr, ".Set") {
		return &Result{}, nil
	}

	currentExpr, ok := buildSetterExpr(info, x.X, false)
	if !ok {
		return &Result{}, nil
	}
	...
	return &Result{
		From: formatNode(x.X),
		To:   fmt.Sprintf("%s(%s %s)", setterExpr, currentExpr, delta),
	}, nil
}

func buildSetterExpr(info *types.Info, expr ast.Expr, final bool) (string, bool) {
	switch x := expr.(type) {
	case *ast.Ident, *ast.BasicLit:
		return formatNode(x), true

	case *ast.ParenExpr:
		s, ok := buildSetterExpr(info, x.X, final)
		if !ok {
			return "", false
		}
		return "(" + s + ")", true
	...
	case *ast.IndexExpr:
		xStr, ok := buildSetterExpr(info, x.X, false)
		if !ok {
			return "", false
		}

		idxStr, ok := buildSetterExpr(info, x.Index, false)
		if !ok {
			return "", false
		}

		return xStr + "[" + idxStr + "]", true
	...
	case *ast.SelectorExpr:
		...
		if final {
			if !isProtoMessage(info, x.X) || !methodIsExists(info, x.X, "Set"+x.Sel.Name) {
				return "", false
			}

			return xStr + ".Set" + x.Sel.Name, true
		}

		if isProtoMessage(info, x.X) && methodIsExists(info, x.X, "Get"+x.Sel.Name) {
			return xStr + ".Get" + x.Sel.Name + "()", true
		}

		return xStr + "." + x.Sel.Name, true
	...
	case *ast.CallExpr, *ast.CompositeLit, *ast.TypeAssertExpr, *ast.ArrayType, *ast.FuncLit, *ast.SliceExpr, *ast.MapType, *ast.BinaryExpr, *ast.UnaryExpr:
		return formatNode(x), true
...
func buildSetterArgExpr(info *types.Info, lhs ast.Expr, rhs ast.Expr) string {
	if se, ok := finalSelectorExpr(lhs); ok {
		if setterTakesPointer, ok := setterParamHasPointer(info, se.X, se.Sel.Name); ok && !setterTakesPointer {
			if ue, ok := rhs.(*ast.UnaryExpr); ok && ue.Op == token.AND {
				return formatNode(ue.X)
			}
		}
	}

	return formatNode(rhs)
}

func buildClearerExpr(info *types.Info, lhs ast.Expr) (string, bool) {
	se, ok := finalSelectorExpr(lhs)
	if !ok {
		return "", false
	}

	setterTakesPointer, ok := setterParamHasPointer(info, se.X, se.Sel.Name)
	if !ok || setterTakesPointer || !methodIsExists(info, se.X, "Clear"+se.Sel.Name) {
		return "", false
	}

	xStr, ok := buildSetterExpr(info, se.X, false)
	if !ok {
		return "", false
	}

	return xStr + ".Clear" + se.Sel.Name, true
}

func finalSelectorExpr(expr ast.Expr) (*ast.SelectorExpr, bool) {
	switch x := expr.(type) {
	case *ast.SelectorExpr:
		return x, true
	case *ast.StarExpr:
		return finalSelectorExpr(x.X)
	case *ast.ParenExpr:
		return finalSelectorExpr(x.X)
	default:
		return nil, false
...
func setterParamHasPointer(info *types.Info, x ast.Expr, name string) (hasPointer, ok bool) {
	if info == nil {
		return false, false
	}
	...
	obj, _, _ := types.LookupFieldOrMethod(t, true, nil, "Set"+name)
	if obj == nil {
		return false, false
	}
	...
	_, ok = params.At(0).Type().(*types.Pointer)
	if !ok {
		return false, true
	}
